...
class CArticleParameters {
    public String slug;
}

@WebServlet("/api/articles/remove-article-reaction")
public class RemoveReaction extends HttpServlet {
        ...
        try {
            if (req.getContentType().equals("application/json")) {
                if (Auth.isLoggedIn(req)) {
                    ...
                    CArticleParameters params = ReqMethods.mapper.readValue(ReqMethods.getBody(req),
                            CArticleParameters.class);
                    Article article = Articles.getArticle(params.slug);
                    Articles.removeReaction(article.id, userId);
                    ResMethods.writeJSONResponse(res, 200,
                            ResMethods.get200ResJSON("Article reaction removed successfully"));
                } else {
